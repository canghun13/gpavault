# GPA Vault 인수인계 문서 v6 (2026-07-13 기준)

이전 v5 문서를 대체함. v5(및 그 이전 v4)의 배경 설명은 그대로 유효하므로 필요시 참고. 이 문서는 **07-13 세션에서 바뀐 것 + 새로 확인된 사실** 위주로 정리.
...
## 5. 최신 커밋 상태
```
fc278c5 Add FAQ + FAQPage schema: how-many-as-to-raise-gpa.html + ib-gpa-calculator.html (07-13)
...
## 9. 알아둬야 할 사실 (student loan 정책, 2026-07 기준)
...
## 10. GSC / GA4 현황 (07-13 기준)
- **색인: 20 / 미색인 합계 35** (리디렉션 3 + noindex 1(정상, 위 8번 참고) + 404 1(검증 중) + 발견-미색인 29 + 크롤링-미색인 1) — 색인 20/발견-미색인 29는 **07-11 대비 변화 없음**, 아직 이른 시점
...
- **GA4 (06-15~07-12, 4주)**: 세션 총 127건 중 Direct 104 / Organic Search 11 / Referral 9 / AI Assistant 1 — **오가닉 유입이 극히 적음**, 트래픽 대부분 직접 방문(사용자 본인 테스트 접속 포함 가능성 있음). 총수익 $0 (AdSense 미연결 또는 아직 승인 전으로 추정 — 사용자에게 AdSense 연결 상태 확인 필요할 수 있음, 다음 세션에 문의)
- Organic 유입이 낮은 이유는 순위 자체가 낮기 때문(대부분 쿼리가 50~100위)이라 GSC 반영 지연 문제라기보다는 **아직 성장 초기 단계**로 보임

## 11. 관찰 중 / 재평가 보류 항목
- `blog/how-to-raise-your-gpa-in-one-semester.html`: 07-07 title 변경 효과 재평가는 최소 07-21 이후로 미룰 것 (v5 기준 유지)
- `college-cost-calculator.html`, `act-score-calculator.html`: 헤드 키워드 경쟁 압도적이라 당분간 관망 (v5 기준 유지)
- `blog/how-many-as-to-raise-gpa.html`, `tools/ib-gpa-calculator.html`: 07-13에 막 보강함, 최소 2주 후 재평가

## 12. 신규 후보 (백로그, 착수 안 함)
- SSAT percentile calculator — 검색 볼륨 작음(월 노출 4 수준), 우선순위 낮음
- IB GPA 클러스터는 07-13에 착수 완료 (위 8번 참고), 백로그에서 제거
- 이번 세션엔 신규 페이지 후보 추가 발굴 안 됨 (기존 21개 tools + 27개 blog로 주요 쿼리 커버리지 양호, 검색 결과 상 뚜렷한 미커버 토픽 없음)

## 13. 파일 현황
- tools: 21개 + index (`sat-percentile-calculator.html`이 가장 최근 신규, 07-07)
- blog: 27개 + index
- 전체 sitemap URL: 53개 (변동 없음, 07-13에도 신규 페이지 추가 안 함, lastmod만 갱신)

## 14. 다음 세션 시작 전 체크리스트
1. 이 문서(v6) 먼저 정독
2. 새 GSC Performance/Coverage export + GA4 export 받아서 07-13 데이터와 비교 (특히 "크롤링됨-미색인 1건" 지속 여부, 색인 20/29 변동 여부, Organic Search 세션 비중 변화)
3. 새 GitHub 토큰 발급받기
...
8. 07-11에 보강한 5개 파일 + 07-13에 보강한 2개 파일(how-many-as-to-raise-gpa, ib-gpa-calculator)은 최소 2주는 재작업하지 말 것 — 색인 반영 시간 필요
9. AdSense 수익 $0 지속 중 — 다음 세션에 AdSense 연결/승인 상태를 사용자에게 확인해볼 것 (트래픽이 붙기 시작해도 광고 미승인 상태면 수익화가 안 됨)
