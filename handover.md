...
## ★ 09-15 세션 사후 수정 — 차트 중복 (커밋 `86d750b`)

사용자가 스크린샷으로 `pell-enrollment-intensity-calculator`의 차트 중복을 지적했다.

**원인**: 금액 차트를 추가할 때 **기존 퍼센트 차트를 확장하지 않고 별도 표를 바로 아래에 덧붙였다.** 두 표의 앞 두 열(Credits / Intensity)이 완전히 동일해, 화면에서 같은 12행이 연달아 두 번 나오는 상태가 됐다.

**조치**: 5열 단일 표로 통합(`Credits / Intensity / Per term / Full year / Old category`). H2 두 개도 하나로 합치고 중복 설명 문단 정리. 1,176 → 1,134단어.

**검증**: jsdom으로 explainer 내 table 1개·5열 13행, 페이지 내 중복 행 0건 확인. 모바일은 `display:block + nowrap + overflow-x:auto`로 가로 스크롤되므로 5열도 안 깨진다(반응형 CSS 미변경).
...
65. **표를 추가하기 전에 "기존 표에 열을 붙이면 되는 문제인가"를 먼저 확인할 것.** 09-15에 금액 차트를 별도 표로 추가해 Credits/Intensity 12행이 두 번 반복되는 사고가 났다. **새 표는 기존 표와 겹치는 열이 없을 때만 추가한다.** 같은 키(여기서는 학점)로 정렬된 데이터라면 거의 항상 열 추가가 정답이다.
66. **본문을 보강한 뒤에는 렌더된 화면 흐름을 한 번 점검할 것.** 태그 밸런스·스키마 동기화·jsdom 동작 검증은 모두 통과했는데도 "같은 표가 두 번 보인다"는 문제는 잡히지 않았다. 구조 검증과 읽는 경험은 별개다 — 섹션 목록(H2 나열)과 표 헤더 목록을 출력해 **중복이 인접해 있지 않은지** 확인하는 것으로 저비용 점검이 가능하다.
